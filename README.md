...
ZMake 是 Zig 构建系统的一个适配层工具，它可以把现有 C/C++ 构建系统无缝融入 Zig 生态。

## 背景

C/C++ 开发者长期缺乏集**包管理**与**构建**于一体的现代工具链。CMake 与 Autotools 仅是构建系统，且设计陈旧，并无内置的包管理能力；包管理长期依赖于手动下载、git submodule 或系统包管理器，体验割裂。
...
> 本例仅作演示用途，[wolfssl-zig](https://github.com/zfl9/wolfssl-zig) 在本文示例的基础上提供了更完善的集成。

使用 `zig fetch --save` 声明依赖，依赖条目会自动写入 `build.zig.zon`：

```bash
# 引入 zmake 本身
zig fetch --save=zmake https://github.com/zfl9/zmake/archive/refs/tags/v1.0.0.tar.gz
...
.{
    ...
    .dependencies = .{
        .zmake = .{
            .url = "https://github.com/zfl9/zmake/archive/refs/tags/v1.0.0.tar.gz",
            .hash = "...",
        },
        .wolfssl = .{
...
pub fn build(b: *std.Build) void {
    ...
    // 引用构建产物中的头文件与库文件
    b.installHeaders(build_out.path(b, "include"));
    b.installLibrary(build_out.path(b, "lib"));
}
```

- ZMake 自动完成 wolfssl 的配置与构建，并与 Zig 的缓存系统紧密集成。
- 执行 `zig build`，编译产出在 `./zig-out/` 目录下，可见 include、lib 等文件。
...
执行构建，返回指向 `$install_prefix` 目录（即 `{include, lib, ...}` 所在目录）的 `LazyPath`。
